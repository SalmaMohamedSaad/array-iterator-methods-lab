...
// Complete the exercise in the space below:
// I used the article in this link https://www.sitepoint.com/sort-an-array-of-objects-in-javascript/#:~:text=To%20sort%20an%20array%20of%20objects%2C%20use%20the%20sort(),or%20any%20other%20custom%20property. to understand how to use the compare function with an array of objects
const sortAscendry = (a, b) => {
  const aYear = a.year
  const bYear = b.year
  let comparison = 0
  if (aYear > bYear) {
...
sortedByBirthYear = inventors.sort(sortAscendry)
// Check your work:
...
firstLast = people.map((person) => {
  let namesArr = []
  let temp = []
  namesArr = person.split(',')
  //console.log(namesArr)
  namesArr.forEach((name, index) => {
    if (index === 0) {
      temp.push(namesArr[index + 1] + ' ' + namesArr[index])
    }
  })
  return temp
})
...
// I used the information about the date object to make the functionality dynamic from this link https://bugfender.com/blog/javascript-date-and-time/#:~:text=the%20current%20date.-,Current%20date%20using%20JavaScript%20Date%20Methods,month%2C%20and%20date%2C%20respectively.&text=We%20can%20now%20combine%20these%20methods%20to%20form%20the%20current%20date%20representation.
const date = new Date()
// Complete the exercise in the space below:
...
totalYearsLived = inventors.reduce(function (acc, currentValue) {
...
// Complete the exercise in the space below:
travelMethodCounts = travelMethods.reduce(function (acc, travelMethod) {
  if (acc[travelMethod]) {
